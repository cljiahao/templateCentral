...
- **Change-narration keywords** (`was`, `added`, `removed`, `changed`, `updated`, `renamed`, `moved`, `refactored`, `per review`, `as requested`, plus date and ticket-reference patterns) are anchored to comment-start to minimize false positives. They apply to every comment form — plain `#`/`//` lines and structured doc-comment blocks (`/** */`, `"""..."""`) alike. A "short-lived" narration problem doesn't stop being one just because it's sitting in a docstring instead of a line comment.
