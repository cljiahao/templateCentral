...
- After cleanup, run that stack's **tests and production build** — `pnpm test` + `pnpm build` for the TypeScript stacks, `pytest` + a container build for FastAPI — the app must still compile and run with no errors.
